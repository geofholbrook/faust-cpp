...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("SineSweep");
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp0 = (fRec1[1] + (fConst1 * ((50.0f * ftbl0mydspSIG0[int((65536.0f * fRec2[0]))]) + 100.0f)));
...
SineSweep::SineSweep(int sample_rate, int buffer_size)
{
    fUI = new MapUI();
...
void SineSweep::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float SineSweep::getParamValue(const std::string& path)
{
...
extern "C" void app_main()
{
    ...
    SineSweep* DSP = new SineSweep(48000, 32);
    DSP->start();
